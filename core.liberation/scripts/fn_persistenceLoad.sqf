...
if (!isNil "_state" && {!(_state isEqualTo objNull)}) then {
    ...
    missionNamespace setVariable ["front_friendlyNaval", [_state, "friendlyNaval", missionNamespace getVariable ["front_friendlyNaval", []]] call _getValue];
    ...
    missionNamespace setVariable ["front_enemyNaval", [_state, "enemyNaval", missionNamespace getVariable ["front_enemyNaval", []]] call _getValue];
    ...
    missionNamespace setVariable ["front_sectorPorts", [_state, "sectorPorts", []] call _getValue];
    missionNamespace setVariable ["front_sectorCarriers", [_state, "sectorCarriers", []] call _getValue];
    ...
    missionNamespace setVariable ["front_ownedPorts", [_state, "ownedPorts", []] call _getValue];
    missionNamespace setVariable ["front_builtStructures", [_state, "builtStructures", []] call _getValue];
    private _placements = [_state, "builtPlacements", []] call _getValue;
    missionNamespace setVariable ["front_builtPlacements", _placements];
    ...
    private _savedFobs = [_state, "friendlyFobs", []] call _getValue;
    missionNamespace setVariable ["front_friendlyFobs", _savedFobs];
    ...
    missionNamespace setVariable ["front_hasNaval", [_state, "hasNaval", missionNamespace getVariable ["front_hasNaval", false]] call _getValue];
    ...
    private _fobPositions = [];
    {
        _x params ["_name", "_class", "_pos", ["_dir", 0]];
        private _obj = createVehicle [_class, _pos, [], 0, "NONE"];
        _obj setPosATL _pos;
        _obj setDir _dir;

        if (_class isEqualTo _structureClass) then {
            _fobPositions pushBack _pos;
            [_pos, _name] call front_fnc_markFob;
        };
    } forEach _placements;

    if ((_savedFobs isEqualTo []) && {!(_fobPositions isEqualTo [])}) then {
        missionNamespace setVariable ["front_friendlyFobs", _fobPositions];
    };
    ...
    diag_log format ["Invasion persistence: restored save '%1'", _saveKey];
} else {
    missionNamespace setVariable ["front_persistenceLoaded", false];
    diag_log format ["Invasion persistence: no save found for '%1'", _saveKey];
};
