...
[] spawn {
    while {true} do {
        private _anchors = missionNamespace getVariable ["front_fobAnchors", ["Land_Cargo_HQ_V1_F"]];
        private _nearFob = objNull;
        private _candidates = nearestObjects [player, _anchors, 20];
        if (!(_candidates isEqualTo [])) then { _nearFob = _candidates select 0; };
