...
- **Logistics & build menu:** `fn_spawnLogisticsMenu` and `config/Buildables.hpp` define FOB-only build tabs (buildings, infantry, vehicles, helicopters, planes, defenses, fortifications) with unlock conditions (airfield ownership, barracks, factory, air control). Additional infantry (engineers/AA), armor (IFV/MBT/AA), turrets, fast movers, and a wide catalog of bunkers, barriers, tents, radars, and towers are available for purchase to match the larger faction pools. Structures are placed at a player-selected map click, stored in persistence, and the logistics menu spends faction resources/fuel/manpower, while the town shop is strictly for personal-cash gear and civilian vehicle buys so infantry creation stays in the FOB build flow. FOB HQs are mobile trucks with a deploy action; the mission spawns an initial deployed HQ away from hostile sectors and every deployed HQ is recorded for the tactical map and battle overlay.
...
- **Naval fronts (when water is present):** `fn_initWorld` flags harbors/shorelines, `fn_setupDefenses` seeds ports/carriers with boats, garrisons, and decked aircraft, and `fn_navalOps` drives patrols plus amphibious landings so both factions can fight across beaches and capture maritime sectors.
- **Sector control rules:** `fn_sectorMonitor` enforces kill-to-clear captures for BLUFOR (surrendered troops excluded), shows in-radius friendly/enemy counts to players inside sectors, and lets OPFOR reclaim sites only after holding them uncontested for a configurable period (5–20 minutes via lobby parameter).
- **Surrender chances:** `fn_surrender` monitors frontline groups for wounds, attrition, and outnumbering; squads may capitulate and enter captive state so POW capture/holding flows stay intact for both sides.
- **Drone warfare:** `fn_drones` spawns suicide and grenade-dropping UAVs for both factions along the frontline, steering them toward contested sectors so ambient AI pressure includes buzzing kamikaze runs and hovering grenade drops.
...
- **AI density parameter:** an “Overall AI Presence” lobby option scales garrison sizes, mission guards, counterattack weights, and combined-arms ambient patrols from 50% to 500% of the baseline numbers.
